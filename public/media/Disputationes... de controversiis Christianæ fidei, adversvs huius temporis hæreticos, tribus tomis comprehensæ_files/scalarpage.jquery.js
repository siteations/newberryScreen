	...
	$.scalarpage = function(e, options) {
		...
		var page = {
				...
				if ( options.showLateralNav ) {
					$( '[rel="scalar:continue_to"]' ).each( function() {
						...
						if (( page.containingPathNodes.length > 0 ) && ( page.containingPathNodes.indexOf( currentNode ) == ( page.containingPathNodes.length - 1 ) )) {
							...
							var end_button = $( '<a class="nav_btn" href="' + node.url + '">End of section &ldquo;' + page.containingPath.getDisplayTitle() + '&rdquo;; <br /> ' + continuePhrase + ' &ldquo;' + node.getDisplayTitle() + '&rdquo;</a>' ).appendTo(links);
							if ( pathOptionCount == 0 ) {
							...
							if ( page.containingPathIndex > 0 ) {
								...
								back_button = $( '<a id="back-btn" class="nav_btn" href="' + page.containingPathNodes[ page.containingPathIndex - 1 ].url + '?path=' + page.containingPath.slug + '">&laquo; Go back</a> ' ).prependTo(links);
							}
							section.append( links );
			...
			addColophon: function() {
				...
				var $footer = $('#footer');
				$footer.append('<div id="block-block-62" class="block block-block address last even"><div class="inner"><div class="content"><ul><li><a href="https://www.newberry.org" target="_blank">The Newberry</a></li><li>60 West Walton Street</li><li>Chicago, Illinois 60610</li><li>(312) 943-9090</li><li><a href="https://www.newberry.org/contact-librarian" target="_blank">Contact Us</a></li></ul></div></div></div>');
					...
					switch (viewType) {
						...
						for ( var i = 0; i < n; i++ ) {
							...
							if ( node.current.description != null ) {
								...
								description += '... <a href='+node.url+' target="_blank">more</a>';
								}
								...
								description = bib_citation + description;
								}
								else {
									description = description;
								}
								// this was making the links weird. we may need to switch it back if it seems like it's having off effects.
								// description = description.replace( new RegExp("\"", "g"), '&quot;' );
								// removed " data-content="' + description + '" from following node
								...
								var elementHeights = $('.description-caption').map(function() {
									return $(this).height();
								}).get();
		...
		if ( currentNode != null && currentNode.current != null) {
			...
			switch (viewType) {
				...
				if (currentNode.current.auxProperties["dcterms:bibliographicCitation"] != null) {
					var bg_cite = currentNode.current.auxProperties["dcterms:bibliographicCitation"];
				$('.title_card').append('<div class="bg_citation" style="float:right;">' + bg_cite + '</div>');
				}
				...
				if ( currentNode.current.auxProperties["dcterms:bibliographicCitation"] != null ) {
					$( '.image_header' ).append( '<div class="bg_header_cite">' + currentNode.current.auxProperties["dcterms:bibliographicCitation"] + '</div>' );
				}
