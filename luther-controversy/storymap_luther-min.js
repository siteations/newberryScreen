/* storymapjs - v0.6.3 - 2016-12-16
 * Copyright (c) 2016 Northwestern University Knight Lab
 */
...
(function(VCO) {
    ...
    (function($) {
        ...
        $.ajax = function(options) {
            ...
            //console.log('can i hardcode this? ', xhr);
        ...
        function parseArguments(url, data, success, dataType) {
            ...
            console.log('parseArguments -url ', url);
            console.log('parseArguments -data ', data);
            console.log('parseArguments -dataType ', this);
            return {
                url: url,
        ...
        $.getJSON = function(url, data, success) {
            ...
            //console.log(typeof($.ajax(options)));
            return $.ajax(options)
            //return coreData
        };
...
VCO.Ease = {
    ...
    mirror: function(pos) {
        if (pos < .5) return easings.sinusoidal(pos * 2);
        else return easings.sinusoidal(1 - (pos - .5) * 2)
    },
...
VCO.Dom = {
    ...
    getTranslateString: function(point) {
        return VCO.Dom.TRANSLATE_OPEN + point.x + "px," + point.y + "px" + VCO.Dom.TRANSLATE_CLOSE
    },
...
VCO.DomUtil = {
    ...
    getViewportOffset: function(element) {
        ...
        do {
            ...
            if (el.offsetParent === docBody && VCO.DomUtil.getStyle(el, "position") === "absolute") {
                break
            }
            el = el.offsetParent
        } while (el);
    ...
    getTranslateString: function(point) {
        return VCO.DomUtil.TRANSLATE_OPEN + point.x + "px," + point.y + "px" + VCO.DomUtil.TRANSLATE_CLOSE
    },
...
VCO.MenuBar = VCO.Class.extend({
	...
	_onButtonBibliography: function(e) {
        this.fire("bibliography", e)
    },
    ...
    _initLayout: function() {
		...
		this._el.button_bibliography = VCO.Dom.create("span", "vco-menubar-button", this._el.container);
        VCO.DomEvent.addListener(this._el.button_bibliography, "click", this._onButtonBibliography, this);
        ...
        if (VCO.Browser.mobile) {
			...
			this._el.button_bibliography.innerHTML = "<span class='vco-icon-bib'></span>";
            this._el.button_collapse_toggle.innerHTML = "<span class='vco-icon-arrow-up'></span>";
            this._el.container.setAttribute("ontouchstart", " ")
        } else {
            this._el.button_backtostart.innerHTML = VCO.Language.buttons.backtostart + " <span class='vco-icon-goback'></span>";
			this._el.button_bibliography.innerHTML = VCO.Language.buttons.bibliography + " <span class='vco-icon-bib'></span>";
            this._el.button_collapse_toggle.innerHTML = VCO.Language.buttons.collapse_toggle + "<span class='vco-icon-arrow-up'></span>"
        }
    ...
    updateMediaDisplay: function(layout) {
        if (this._state.loaded) {
            ...
            if (!VCO.Browser.mobile && layout != "portrait") {
                this._el.content_item.style.maxHeight = this.options.height / 2.5 + "px"
            }
...
VCO.Media.Image = VCO.Media.extend({
    ...
    _loadMedia: function() {
        ...
        if (this.data.link) {
            ...
            this._el.content_link.target = "_blank";
            this._el.content_item = VCO.Dom.create("img", "vco-media-item vco-media-image", this._el.content_link) //removed vco-media-shadow from here (after vco-media-image), add it back to add shadow again
        } else {
            this._el.content_item = VCO.Dom.create("img", "vco-media-item vco-media-image", this._el.content) //removed vco-media-shadow from here (after vco-media-image), add it back to add shadow again
        }
        this._el.content_item.addEventListener("load", function(e) {
            self.onMediaLoaded()
        });
...
VCO.Media.Text = VCO.Class.extend({
    ...
    _initLayout: function() {
		...
		// subtitle class added by MK
		if (this.data.subtitle != "") {
            var subtitle_class = "subtitle";

            this._el.subtitle = VCO.Dom.create("div", subtitle_class, this._el.content_container);
            this._el.subtitle.innerHTML = this.data.subtitle
        }
        if (this.data.text != "") {
            var text_content = "";
		...
		// citation class added by MK
		if (this.data.citation != "") {
            var citation_class = "vco-citation";
            this._el.citation = VCO.Dom.create("p", citation_class, this._el.content_container);
            this._el.citation.innerHTML = this.data.citation
        }
		if (this.data.cataloglink != "" && this.data.digilink != "") {
            var cataloglink_class = "vco-viewmore";
            this._el.cataloglink = VCO.Dom.create("p", cataloglink_class, this._el.content_container);
            this._el.cataloglink.innerHTML = 'View: <a href=' +this.data.digilink+ ' target="_blank">Digitized item</a> | <a href='+ this.data.cataloglink + ' target="_blank"> Catalog record</a>'
        }
		if (this.data.cataloglink != "" && this.data.digiselection != "") {
            var cataloglink_class = "vco-viewmore";
            this._el.cataloglink = VCO.Dom.create("p", cataloglink_class, this._el.content_container);
            this._el.cataloglink.innerHTML = 'View: <a href=' +this.data.digiselection+ ' target="_blank">Digitized selections</a> | <a href='+ this.data.cataloglink + ' target="_blank"> Catalog record</a>'
        }
        }
        this.onLoaded()
    }
...
VCO.Media.Wikipedia = VCO.Media.extend({
    ...
    createMedia: function(d) {
        ...
        if (d.query) {
            ...
            content += wiki.text;
            if (wiki.extract.match("REDIRECT")) {} else {
                this._el.content_item.innerHTML = content;
                this.onLoaded()
            }
        }
...
VCO.Slide = VCO.Class.extend({
    ...
    _initLayout: function() {
		...
		// below line changed by MK, originally this._el.container at end
        this._el.background = VCO.Dom.create("div", "vco-slide-background", this._el.content_container);
        if (this.data.background) {
...
VCO.StorySlider = VCO.Class.extend({
    ...
    showNav: function(nav_obj, show) {
        if (this.options.width <= 500 && VCO.Browser.mobile) {} else {
            if (show) {
                nav_obj.show()
            } else {
                nav_obj.hide()
    ...
    changeBackground: function(bg) {
        ...
        var bg_color = {
            ...
            bg_color_rgb, bg_percent_start = this.options.slide_default_fade,
            bg_percent_end = "15%",
            bg_alpha_end = "0.87",
            bg_css = "",
    ...
    _introInterface: function() {
        ...
        if (this.options.width <= this.options.skinny_size) {} else {
            this._nav.next.updatePosition({
                right: "130"
            }, false, this.options.duration * 3, this.options.ease, -100, true);
            this._nav.previous.updatePosition({
                left: "-100"
            }, true, this.options.duration * 3, this.options.ease, -200, true)
        }
    },
    _initLayout: function() {
...
L.Icon = L.Class.extend({
    ...
    _setIconStyles: function(img, name) {
		...
		var node = document.createTextNode("test text");
...
L.Marker = L.Class.extend({
    ...
    _initIcon: function() {
        ...
        if (options.riseOnHover) {
            L.DomEvent.on(icon, "mouseover", this._bringToFront, this).on(icon, "mouseout", this._resetZIndex, this).on(icon, "mouseover", this._hoverOver, this).on(icon,"mouseout",this._hoverOff, this)
        }
    ...
    _removeIcon: function() {
        if (this.options.riseOnHover) {
            L.DomEvent.off(this._icon, "mouseover", this._bringToFront).off(this._icon, "mouseout", this._resetZIndex).off(icon, "mouseover", this._hoverOver, this).off(icon, "mouseout", this._hoverOff, this)
        }
	...
	_hoverOver: function() {
		var icon = this._icon
		L.DomUtil.addClass(icon, "leaflet-hovered");
	},
	_hoverOff: function() {
		var icon = this._icon
		L.DomUtil.removeClass(icon, "leaflet-hovered");
	},
	_makeHidden: function() {
		var icon = this._icon
		L.DomUtil.addClass(icon, "hidden-icons");
	},
	...
	_hoverText: function() {
		...
		var icon = this._icon
		L.DomUtil.appendChild(icon,para);
	}
...
L.PolyUtil.clipPolygon = function(points, bounds) {
    var clippedPoints, edges = [1, 4, 2, 8],
        i, j, k, a, b, len, edge, p, lu = L.LineUtil;
...
VCO.MapMarker.Leaflet = VCO.MapMarker.extend({
    ...
    _createIcon: function(active) {
		var className = active ? "vco-clicked-active" : "vco-clicked";
        return new L.icon({
    ...
    _createDefaultIcon: function(active) {
		...
		//below portion added by MK 6/6/17
		//todo: this works but it adds 'hidden-marker' class four times(?). Need to figure out how to get it only once.
		var cols = document.getElementsByClassName('vco-mapmarker');
		 for(i=0; i<cols.length; i++) {
    cols[i].className += ' hidden-marker';
  }
        return L.divIcon({
...
VCO.Map.Leaflet = VCO.Map.extend({
        ...
        switch (_map_type_arr[0]) {
            case "mapbox":
                if (mapbox_name = _map_type_arr[1]) {
                    ...
                    _options.attribution = _attribution_knightlab + "<div class='mapbox-maplogo'></div><a href='https://www.mapbox.com/about/maps/' target='_blank'>Â© Mapbox Â© OpenStreetMap</a>";
            ...
            case "zoomify":
                ...
                _options.tolerance = this.options.zoomify.tolerance || .9;
            ...
            case "osm":
                ...
                _options.attribution = _attribution_knightlab + "Â© <a target='_blank' href='http://www.openstreetmap.org'>OpenStreetMap</a> and contributors, under an <a target='_blank' href='http://www.openstreetmap.org/copyright'>open license</a>";
    ...
    _markerOverview: function() {
        ...
        if (this.options.map_type == "zoomify" && this.options.map_as_image) {
            ...
            if (this.options.map_center_offset && this.options.map_center_offset.left != 0 || this.options.map_center_offset.top != 0) {
				...
				// parts below changed by mk, normal is _zoom - 1
                _location = this._getMapCenterOffset(_location, _zoom - 0);
                this._map.setView(_location, _zoom - 0, {
                    pan: {
                        animate: true,
                        duration: this.options.duration / 1e3,
                        easeLinearity: .1
        ...
        for (var i = 0; i < this._markers.length; i++) {
            if (this._markers[i].data.location) {
                var marker = this._markers[i],
                    prev_marker, next_marker, marker_location, prev_marker_zoom, next_marker_zoom, calculated_zoom;
                if (marker.data.type && marker.data.type == "overview") {
    ...
    _viewTo: function(loc, opts) {
		...
		// change opacity to toggle line visibility when map_as_image is false -MK
        if (!this.options.map_as_image) {
            this._line_active.setStyle({
                opacity: 1
            })
...
VCO.StoryMap = VCO.Class.extend({
    ...
    _initEvents: function() {
		...
		this._menubar.on("bibliography", this._onBibliography, this);
	...
	_onBibliography: function(e) {
        this.current_slide = 26;
        this._map.goTo(this.current_slide);
        this._storyslider.goTo(this.current_slide);
        this.fire("change", {
            current_slide: this.current_slide
...
$( document ).ready(function(){
    $('body').find('.leaflet-marker-icon').css('display', 'none');
	console.log('blah');
});
